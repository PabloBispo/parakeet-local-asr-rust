...
RUN mkdir /out \
    && cp target/release/parakeet-local-asr-rust /out/ \
...
COPY --from=builder /out/parakeet-local-asr-rust /usr/local/bin/parakeet-local-asr-rust
...
ENV MODELS_DIR=/models \
    ...
    ASR_DEVICE=cpu \
    RUST_LOG=parakeet_local_asr_rust=info

VOLUME ["/models"]
EXPOSE 8090
ENTRYPOINT ["parakeet-local-asr-rust"]
